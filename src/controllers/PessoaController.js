...
class PessoaController extends Controller {
  ...
  async pegaMatriculasAtivas(req, res) {
    ...
    try {
      const listaMatriculas = await pessoaServices.pegaMatriculasAtivaPorEstudante(Number(estudante_id));
  ...
  async cancelaRegistroEstudante(req, res) {
    ...
    try {
      ...
      return res.status(200).json({ messagem: `matriculas ref. estudante ${estudante_id} canceladas.`});
    } catch (erro) {
      return res.status(500).json({ erro: erro.message });
    }
  }
